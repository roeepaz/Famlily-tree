...
import { UserPlus, Clock, Check, X, TreePine } from "lucide-react";
...
function TreeSkeleton() {
  return (
    <div className="space-y-10 animate-pulse w-full max-w-4xl mx-auto py-4">
      {[1, 2, 3].map((rowIdx) => (
        <div key={rowIdx}>
          ...
          <div className="flex justify-center gap-4">
            {Array.from({ length: rowIdx === 2 ? 3 : 2 }).map((_, nodeIdx) => (
              <div
                key={nodeIdx}
                className="flex flex-col items-center gap-2 p-4 rounded-2xl border border-slate-800/80 bg-slate-900/40 w-36 text-center"
              >
                <div className="w-14 h-14 rounded-full bg-slate-800 shrink-0" />
                <div className="h-3 w-16 bg-slate-800 rounded-full mt-1" />
              ...
              {isEditMode && (
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                  <Button 
                    ...
                    variant="outline"
                    className="w-full sm:w-auto rounded-xl gap-2 border-teal-500/30 text-teal-400 hover:bg-teal-500/10 hover:text-teal-300 font-medium px-5 shadow-sm bg-slate-900"
                  >
                    <span>🌱</span>
                  ...
                  <Button 
                    ...
                    className="w-full sm:w-auto rounded-xl gap-2 font-medium px-5 shadow-sm bg-teal-500 hover:bg-teal-600 text-slate-950 font-semibold text-xs flex items-center gap-1.5 shadow-sm transition-all duration-200 hover:scale-[1.02] border-0"
                  >
                    <UserPlus className="w-4 h-4" />
                    הוסף קרוב משפחה ידנית
                  </Button>
                </div>
              )}
            </div>
            ...
            {visibleGenerations.map((gen, index) => {
              ...
              return (
                <div key={gen.key}>
                  {/* Generation label */}
                  <div className="flex items-center gap-3 mb-5">
                    <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">{gen.label}</span>
                    <div className="flex-1 h-px bg-slate-800/80" />
                  </div>

                  {/* Nodes Render Container */}
                  <div className="flex flex-nowrap sm:flex-wrap overflow-x-auto sm:overflow-x-visible pb-3 sm:pb-0 justify-start sm:justify-center items-center gap-6 px-4 sm:px-0 no-scrollbar">
                    {(() => {
                      ...
                      return (
                        <>
                          ...
                          {couples.map((couple, cIdx) => (
                            <div 
                              key={`couple-${cIdx}`}
                              className="flex items-center gap-2 p-3.5 border border-dashed border-teal-500/25 rounded-3xl bg-slate-950/40 shadow-inner relative"
                            >
                              <RelativeNodeCard
                                ...
                                isDark={true}
                              />
                              <div className="w-6 h-px bg-gradient-to-r from-teal-500/20 via-teal-500 to-teal-500/20 relative flex items-center justify-center shrink-0">
                                <span className="text-[10px] leading-none text-rose-500 bg-slate-950 px-1 rounded-full border border-teal-500/10 shadow-sm animate-pulse">❤️</span>
                              </div>
                              <RelativeNodeCard
